...
public class PlayerMovementScript : MonoBehaviour {
	...
	*/
	...
	// Declare jetpack-specific variables
	...
	[Tooltip("Fuel recharge rate per second")]
	public float fuelRechargeRate = 10f;
	[Tooltip("Thrust force of the jetpack")]
	public float jetpackThrust = 1000f;
	[Tooltip("Jetpack particle effect")]
	public ParticleSystem jetpackEffect;

	private bool isUsingJetpack;
	void JetpackSystem()
	{
		// Check if the player activates the jetpack
		if (Input.GetKey(KeyCode.Space) && currentFuel > 0)
		{
			isUsingJetpack = true;

			// Apply upward force
			rb.AddForce(Vector3.up * jetpackThrust * Time.deltaTime, ForceMode.Acceleration);

			// Reduce fuel based on usage
			currentFuel -= fuelConsumptionRate * Time.deltaTime;
			currentFuel = Mathf.Clamp(currentFuel, 0, maxFuel);

			// Play jetpack particle effect if available
			if (jetpackEffect && !jetpackEffect.isPlaying)
			{
				jetpackEffect.Play();
			}
		}
		else
		{
			...
			// Recharge fuel if the jetpack is not being used and grounded
			if (grounded)
			{
    ...
    [Tooltip("HUD Fuel  to display bullet count on screen. Will be find under name 'HUD_bullets' in scene.")]
    public TextMesh HUD_Fuel;
    void OnGUI()
    {
        if (!HUD_Fuel)
        {
            try
            {
                HUD_Fuel = GameObject.Find("HUD_Fuel").GetComponent<TextMesh>();
            }
            catch (System.Exception ex)
            {
                print("Couldnt find the HUD_Fuel ->" + ex.StackTrace.ToString());
            }
        }
        if (HUD_Fuel)
            HUD_Fuel.text = "Jetpack Fuel ".ToString() + " - " + currentFuel.ToString();
